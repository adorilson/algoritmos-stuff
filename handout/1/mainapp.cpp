...
int main()
{
  ...
  /*
   * Instantiate the sorting objects.
   */
  ...
  for ( int i = begin_power ; i <= end_power ; i++ ) {
	...
	if ( a[ j ] < a[ j - 1 ] ) {
      ...
      for ( int j = 1 ; j < n ; j++ ) {
        if ( b[ j ] < b[ j - 1 ] ) {
          cerr << endl << "Your implementation of the quicksort algorithm is incorrect!" << endl ;
          return EXIT_FAILURE ;
        }
      }
    ...
    cerr << "Release the memory allocated for the arrays " 
         << endl ;
    /*
